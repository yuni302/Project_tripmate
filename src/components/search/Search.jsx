...
import { SearchStyle, Button } from '../../style/searchstyle/SearchStyle';
...
function Search() {
	...
	const PopularRegion = popularRegionList.map((region) => (
		<li
			key={region}
			onClick={(e) => {
	...
	const GroupType = groupList.map((group) => (
		<button
			...
			type="button"
			checked={checkedInputs.includes(Object.keys(group)[0]) ? true : false}
			onClick={(e) => {
